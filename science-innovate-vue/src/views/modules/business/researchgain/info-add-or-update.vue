<template>
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()"
      ...
      <el-form-item label="参与学生" prop="entryStu">
        <el-input v-model="dataForm.entryStu" placeholder="参与学生"></el-input>
      </el-form-item>
      ...
      <el-form-item label="成果类型" prop="gain">
        <el-select v-model="dataForm.gain" placeholder="请选择">
          <el-option v-for="item in gainTypeList" :key="item.value" :label="item.label" :value="item.value"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="成果级别" prop="grade">
      ...
      <el-form-item label="成果等次" prop="rank">
        <el-select v-model="dataForm.rank" placeholder="请选择">
          <el-option v-for="item in competitionRankList" :key="item.value" :label="item.label"
                     :value="item.value"></el-option>
        </el-select>
      </el-form-item>
    ...
    data () {
      ...
      return {
        ...
        dataRule: {
          ...
          // grade: [
          //   { required: true, message: '成果级别不能为空', trigger: 'blur' }
          // ],
          // rank: [
          //   { required: true, message: '成果等次不能为空', trigger: 'blur' }
          // ],
          personId: [
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (!this.dataForm.rgId) {
            // 新增时 已选成员为空
            this.choiceDataList = []
            this.dataFormLoading = false
          } else {
            this.$http({
              url: this.$http.adornUrl(`/business/researchgain/info/${this.dataForm.rgId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.name = data.researchGain.name
                this.dataForm.entryStu = data.researchGain.entryStu
                this.dataForm.awardTime = data.researchGain.awardTime
                this.dataForm.gain = data.researchGain.gain
                this.dataForm.grade = data.researchGain.grade
                this.dataForm.rank = data.researchGain.rank
                // 已加入项目成员的teacherId
                this.dataForm.personId = data.researchGain.personId
                this.choiceDataList = data.researchGain.memberIdList
                // 附件信息
                this.fileList = transferDataToFile(data.researchGain.sysAttachEntityList)
                this.dataFormLoading = false
              } else {
                this.dataFormLoading = false
              }
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/business/researchgain/${!this.dataForm.rgId ? 'save' : 'update'}`),
              method: 'post',
              data: this.$http.adornData({
                'rgId': this.dataForm.rgId || undefined,
                'name': this.dataForm.name,
                'entryStu': this.dataForm.entryStu,
                'awardTime': this.dataForm.awardTime,
                'gain': this.dataForm.gain,

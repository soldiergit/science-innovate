    ...
    <el-row>
      <table border="1" cellspacing="0" width="100%" class="table">
        ...
        <tr style="height: 2.5rem" align='center'>
          ...
          <td colspan="3"><el-tag style="font-size: 1.1rem; font-weight: bolder" v-text="dataForm.name" size="small"></el-tag></td>
          <th colspan="1">参与学生</th>
          <td colspan="3"><span v-text="dataForm.entryStu ? dataForm.entryStu : '暂无'"></span></td>
        </tr>
        <tr align='center'>
        </tr>
        <tr align='center'>
          <th>成果类型</th>
          <td v-for="item in gainTypeList" v-if="item.value === dataForm.gain">
    ...
    methods: {
      init (id) {
        ...
        if (this.dataForm.rgId) {
          this.$http({
            url: this.$http.adornUrl(`/business/researchgain/details/${this.dataForm.rgId}`),
            method: 'get',
            ...
            if (data && data.code === 0) {
              ...
              // 附件信息
              this.fileList = data.researchGain.sysAttachEntityList
              // 教师信息
              this.personInfo = data.researchGain.personInfo
              this.memberInfoList = data.researchGain.memberInfoList

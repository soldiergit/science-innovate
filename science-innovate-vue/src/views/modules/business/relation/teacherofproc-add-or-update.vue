<template>
  <el-dialog
    ...
    <el-form-item label="项目立项id" prop="pcId">
      <el-input v-model="dataForm.pcId" placeholder="项目立项id"></el-input>
    </el-form-item>
...
<script>
  export default {
    ...
    methods: {
      init (id) {
        this.dataForm.topcId = id || 0
        this.visible = true
        this.dataFormLoading = true
        this.$nextTick(() => {
          this.$refs['dataForm'].resetFields()
          if (this.dataForm.topcId) {
            this.$http({
              url: this.$http.adornUrl(`/business/teacherofproc/info/${this.dataForm.topcId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.teaId = data.teacherOfProc.teaId
                this.dataForm.pcId = data.teacherOfProc.pcId
                this.dataForm.type = data.teacherOfProc.type
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            this.$http({
              url: this.$http.adornUrl(`/business/teacherofproc/${!this.dataForm.topcId ? 'save' : 'update'}`),
              method: 'post',
              data: this.$http.adornData({
                'topcId': this.dataForm.topcId || undefined,

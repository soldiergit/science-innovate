<template>
  <el-dialog
    ...
    <el-form-item label="教学荣誉id" prop="thId">
      <el-input v-model="dataForm.thId" placeholder="教学荣誉id"></el-input>
    </el-form-item>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          thId: [
            { required: true, message: '教学荣誉id不能为空', trigger: 'blur' }
          ],
          type: [
      ...
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.totaId) {
            this.$http({
              url: this.$http.adornUrl(`/business/teacherofth/info/${this.dataForm.totaId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.teaId = data.teacherOfTh.teaId
                this.dataForm.thId = data.teacherOfTh.thId
                this.dataForm.type = data.teacherOfTh.type
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            this.$http({
              url: this.$http.adornUrl(`/business/teacherofth/${!this.dataForm.totaId ? 'save' : 'update'}`),
              method: 'post',
              data: this.$http.adornData({
                'totaId': this.dataForm.totaId || undefined,
                'teaId': this.dataForm.teaId,
                'thId': this.dataForm.thId,

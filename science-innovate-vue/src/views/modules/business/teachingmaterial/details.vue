    ...
    <el-row>
      <table border="1" cellspacing="0" width="100%" class="table">
        ...
        <tr align='center'>
          ...
          <th>出版社</th>
          <td><span v-text="dataForm.press"></span></td>
          <th>出版时间</th>
          <td><span v-if="dataForm.publishTime" v-text="dataForm.publishTime.substring(0,10)"></span></td>
          <th>备注</th>
          <td><span v-text="dataForm.remarks"></span></td>
        </tr>
        ...
        <tr class="contents"><th colspan="8">教材作者成员信息</th></tr>
        <tr align='center'>
          <td colspan="8" style="height: 1.2rem"></td>
        </tr>
        <tr align='center'>
    ...
    methods: {
      init (id) {
        ...
        if (this.dataForm.materialId) {
          this.$http({
            url: this.$http.adornUrl(`/business/teachingmaterial/details/${this.dataForm.materialId}`),
            method: 'get',
            params: this.$http.adornParams()
          }).then(({data}) => {
            if (data && data.code === 0) {
              this.dataForm.name = data.teachingMaterial.name
              this.dataForm.press = data.teachingMaterial.press
              this.dataForm.publishTime = data.teachingMaterial.publishTime
              this.dataForm.remarks = data.teachingMaterial.remarks
              // 附件信息
              this.fileList = data.teachingMaterial.sysAttachEntityList
              // 教师信息
              this.personInfo = data.teachingMaterial.personInfo
              this.memberInfoList = data.teachingMaterial.memberInfoList

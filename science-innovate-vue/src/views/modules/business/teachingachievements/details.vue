    ...
    methods: {
      init (id) {
        ...
        if (this.dataForm.taId) {
          this.$http({
            url: this.$http.adornUrl(`/business/teachingachievements/details/${this.dataForm.taId}`),
            method: 'get',
            ...
            if (data && data.code === 0) {
              ...
              // 附件信息
              this.fileList = data.teachingAchievements.sysAttachEntityList
              // 教师信息
              this.personInfo = data.teachingAchievements.personInfo
              this.memberInfoList = data.teachingAchievements.memberInfoList

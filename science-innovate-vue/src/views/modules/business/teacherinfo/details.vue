  ...
  export default {
    ...
    methods: {
      init (id) {
        ...
        this.teacherInfoEntity.teacherId = id || 0
        // 重置数据
        ...
        if (this.teacherInfoEntity.teacherId) {
          this.$http({
            url: this.$http.adornUrl(`/business/teacherinfo/info/${this.teacherInfoEntity.teacherId}`),
            method: 'get',
      ...
      exportXlsxHandle () {
        this.$confirm(`确定要进行[导出]操作?`, '提示', {
          ...
          setTimeout(() => {
            ...
            try {
              FileSaver.saveAs(new Blob([wbout], {type: 'application/octet-stream'}), this.teacherInfoEntity.name + '的档案信息.xlsx')
            } catch (e) {
